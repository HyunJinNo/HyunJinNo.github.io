...
3. `자식 노드 비교`
   ...
   <b>key</b> 값을 설정할 때 배열의 index를 사용하면 안됩니다. 이는 요소들의 순서가 변경되거나 특정 요소가 삭제될 때 리액트가 요소들을 잘못 인식할 수 있기 때문입니다. 따라서, 배열 내에서 순서가 변경되어도 각 요소의 <b>key</b> 값이 변경되지 않는 고유한 값을 설정해야 합니다. 다만, 변경이 절대 발생하지 않는 정적 리스트인 경우라면 index를 사용해도 무방합니다.
   </blockquote>
