...
processes 레이어는 여러 페이지나 기능에 걸친 복잡한 비즈니스 로직을 처리하는 계층으로 현재는 더 이상 사용되지 않습니다.</p></blockquote>
...
├── pages     # URL 경로에 매핑되는 페이지 컴포넌트를 모아두는 레이어입니다. (Ex. HomePage, SettingsPage 등)
├── widgets   # 여러 페이지에서 공용으로 사용하는 독립적인 UI 컴포넌트를 모아두는 레이어입니다. (Ex. 헤더 네비게이션, 검색 필터 등)
...
- <a href="https://feature-sliced.design/kr/" target="_blank">Welcome | Feature-Sliced Design</a>
