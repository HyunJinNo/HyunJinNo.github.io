...
`PurgeCSS`는 <b>사용되지 않는 CSS를 자동으로 제거해 CSS 파일 크기를 줄여주는 도구</b>입니다. 파일에 들어 있는 모든 키워드를 추출하여 해당 키워드를 이름으로 갖는 CSS 클래스만 보존하고 나머지 매칭되지 않는 클래스는 모두 지우는 방식으로 CSS 파일을 최적화합니다.
