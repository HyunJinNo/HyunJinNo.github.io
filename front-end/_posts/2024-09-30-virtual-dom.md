---
layout: post
title: 가상 돔(Virtual DOM)과 렌더링
description: >
  가상 돔(Virtual DOM)과 렌더링에 대해 설명하는 페이지입니다.
image:
...
- [가상 돔(Virtual DOM)과 렌더링](#가상-돔virtual-dom과-렌더링)
...
이번 글에서는 가상 돔(Virtual DOM)의 개념에 대해 설명한 후, 리액트(React)에서 가상 돔을 활용하여 렌더링하는 방식에 대해 설명하겠습니다.
...
## 가상 돔(Virtual DOM)과 렌더링

`리액트(React)`에서 가상 돔을 활용하여 렌더링하는 방식은 다음과 같습니다.
...
`Diff 알고리즘(또는 Diffing 알고리즘)`이란 새로운 가상 돔과 기존 가상 돔을 비교하여 변경 사항을 찾는 알고리즘입니다. 해당 알고리즘은 크게 다음 3가지 단계로 나뉩니다.
