...
- [Step 3 - Ollama 명령어 사용하기](#step-3---ollama-명령어-사용하기)
  ...
  - [다운로드받은 LLM 목록 확인 - ollama list](#다운로드받은-llm-목록-확인---ollama-list)
...
- [Step 4 - 모델 설치하기](#step-4---모델-설치하기)
- [Step 5 - 모델 실행하기](#step-5---모델-실행하기)
- [참고 자료](#참고-자료)
- [Comments](#comments)
...
Docker로 Ollama를 사용하기 위해선 먼저 Docker를 설치해야 합니다. Docker 설치 방법은 [Docker의 개념과 사용 방법](/raspberry-pi/2024-11-06-docker/) 문서를 참고하시길 바랍니다. Docker가 설치되어 있다면 다음 명령어를 입력하여 Docker Hub에서 Ollama 공식 이미지를 다운로드받을 수 있습니다.
...
이 문단에서는 주요 ollama 명령어에 대해 설명합니다. Docker로 ollama를 사용하는 경우 각 명령어 앞에 `docker exec -it ollama`를 추가로 작성하면 됩니다. (Ex. `docker exec -it ollama` ollama list)

### Ollama 실행 - ollama serve

`ollama serve` 명령어를 입력하여 ollama를 실행할 수 있습니다. Docker로 Ollama를 설치한 경우 `docker run` 명령어 시점에 자동으로 실행됩니다.
...
`ollama pull [모델명]` 명령어를 입력하여 LLM을 다운로드 받을 수 있습니다. 또한 해당 명령어는 로컬 모델을 업데이트할 때도 사용될 수 있습니다.

```bash
ollama pull llama3.2
```

### 다운로드받은 LLM 목록 확인 - ollama list

`ollama list` 명령어를 입력하여 로컬에 다운로드받은 LLM 목록을 확인할 수 있습니다.
...
`ollama show [모델명]` 명령어를 입력하여 로컬에 다운로드받은 특정 LLM 정보를 확인할 수 있습니다.
...
`ollama run [모델명]` 명령어를 입력하여 LLM을 실행할 수 있습니다. 만약 로컬에 실행하고자 하는 LLM이 없는 경우 자동으로 다운로드 받습니다.
...
Ollama를 실행한 후 LLM을 사용하기 위해선 먼저 LLM를 다운로드 받아야 합니다. 이번 글에서는 `Llama 3.2(3B, 2.0GM)`를 다운로드 받아서 실행하겠습니다. 다음 명령어를 입력하여 `Llama 3.2`을 다운로드 받습니다.
